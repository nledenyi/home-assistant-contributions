...
| Memory leak | ha_toyota | [`bug/memory-leak-direct-await`](https://github.com/nledenyi/ha_toyota/tree/bug/memory-leak-direct-await) | [ha_toyota#282](https://github.com/pytoyoda/ha_toyota/issues/282) | [ha_toyota#283](https://github.com/pytoyoda/ha_toyota/pull/283) **MERGED** 2026-04-27 17:14Z by deejay1. Reporter @alpy-nz and @Paja-git both confirmed the leak fix; the residual ramp @arhimidis64 saw was traced to per-request `httpx` allocation and fixed in pytoyoda#252. **HACS users still on v2.2.2** (March release, predates #283) hit the regression with auto-updated pytoyoda 5.1.0 until a new release tag is cut. |
...
| Smart status-refresh strategy + #87 null-render | ha_toyota | `rate-limit-resilience` | [ha_toyota#87](https://github.com/pytoyoda/ha_toyota/issues/87), [#137](https://github.com/pytoyoda/ha_toyota/issues/137), [#157](https://github.com/pytoyoda/ha_toyota/issues/157), [#168](https://github.com/pytoyoda/ha_toyota/issues/168), [#190](https://github.com/pytoyoda/ha_toyota/issues/190), [#229](https://github.com/pytoyoda/ha_toyota/issues/229), [#284](https://github.com/pytoyoda/ha_toyota/issues/284) | [ha_toyota#286](https://github.com/pytoyoda/ha_toyota/pull/286) **MERGED** 2026-04-28 08:06:58Z by deejay1 (squash `45c7f78`). Per-VIN per-cycle decision tree, retain-on-transient-failure, options-flow, 5 new diagnostic sensors, fault isolation, endpoint-tagged 429 logs. Last commit before merge: services.yaml hassfest fix (`device` selector under `fields:` instead of inline target filter). See [`smart-status-refresh.md`](smart-status-refresh.md). **HACS users still on v2.2.2** until a new release tag is cut. |
...
| Climate-settings 500 resilience (per-endpoint optional) | pytoyoda | [`feat/per-endpoint-optional`](https://github.com/nledenyi/pytoyoda/tree/feat/per-endpoint-optional) | [ha_toyota#291](https://github.com/pytoyoda/ha_toyota/issues/291) (Bucket 1, sciurius) | [pytoyoda#258](https://github.com/pytoyoda/pytoyoda/pull/258) draft. Some accounts get persistent HTTP 500 on `GET /v1/global/remote/climate-settings` with no client-visible flag predicting it. Falsified the "smoking gun" hypothesis with a 4-vehicle empirical dataset; landed an `optional=True` flag on `EndpointDefinition` so individual endpoint failures no longer abort the whole `Vehicle.update()` cycle. Tester @sciurius confirmed end-to-end ("BINGO!"). See [`issue-291-resilience.md`](issue-291-resilience.md). |
| Smart-strategy resilience (refresh-status 5xx) | ha_toyota | [`fix/wake-post-500-auto-disable`](https://github.com/nledenyi/ha_toyota/tree/fix/wake-post-500-auto-disable) | [ha_toyota#293](https://github.com/pytoyoda/ha_toyota/issues/293) (spun out of #291 Bucket 2, @Nickduino + @LukasVogel123) | [ha_toyota#295](https://github.com/pytoyoda/ha_toyota/pull/295) draft. The smart-strategy v2.3.0 work (#286) didn't auto-disable when the wake POST 500'd; same exception ALSO froze entities at last-good. Single root cause (missing try/except around the POST that propagated past bookkeeping), two simultaneous symptoms. Bundled fix: wrap+collapse failure paths, bare GET fallback, service-call bypass for both auto- and user-disable, auto-clear on successful POST. See [`issue-291-resilience.md`](issue-291-resilience.md). |
